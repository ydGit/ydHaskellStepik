...
instance Monad (FailCont r e) where
  ...
  (FailCont v) >>= k = undefined --FailCont $ runFailCont . k $ v
...
callCC f = Cont $ \c -> runCont (f $ \v -> Cont $ \_ -> c v) c


-- newtype FailCont r e a = FailCont { runFailCont :: (a -> Either e r) -> (a -> Either e r) -> Either e r}
callCFC :: ((a -> FailCont r e b) -> FailCont r e a) -> FailCont r e a
callCFC f = FailCont $ \cok cer -> runFailCont (f $ \v -> FailCont $ \_ cerr -> cok v) cok cer
-- ((a -> FailCont r e1 a) -> FailCont r2 e2 a2) -> FailCont r2 e2 a2
